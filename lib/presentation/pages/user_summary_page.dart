    ...
    return BlocBuilder<UserSummaryCubit, UserSummaryState>(
      builder: (context, state) {
        if (state.isLoading && state.user == null) {
          return Scaffold(body: Center(child: AppLoading(message: 'Cargando usuario...')));
        }
        if (state.failure != null) {
          ...
          return Scaffold(appBar: AppBar(title: const Text('Resumen del Usuario')), body: Center(child: Text('Error: $msg')));
        }
        final User? user = state.user;
        if (user == null) {
          return Scaffold(appBar: AppBar(title: const Text('Resumen del Usuario')), body: const Center(child: Text('Usuario no encontrado')));
        }
  final theme = Theme.of(context);
  final onSurfaceSubtle = theme.colorScheme.onSurface.withOpacity(0.7);
        final fecha = user.fechaNacimiento.toLocal().toString().split(' ').first;

        return Scaffold(
          appBar: AppBar(title: const Text('Resumen del Usuario'), centerTitle: true),
          body: SafeArea(
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('ID: ${user.id}', style: theme.textTheme.titleMedium?.copyWith(color: onSurfaceSubtle)),
                  const SizedBox(height: 4),
                  Text('Fecha de nacimiento: $fecha', style: theme.textTheme.bodyMedium?.copyWith(color: onSurfaceSubtle)),
                    ...
                    children: [
                      Expanded(
                        child: FilledButton.tonal(
                          onPressed: () async {
                            final result = await context.push('${NamesRouter.createEditUser}?id=${user.id}');
                      ...
                      Expanded(
                        child: ConfirmDeleteButton(
                          ...
                          message: 'Esta acción eliminará el usuario y sus direcciones. ¿Deseas continuar?',
                          onConfirmed: () async {
                  ...
                  if (state.addresses.isEmpty) ...[
                    Text(
                      ...
                      style: theme.textTheme.bodyMedium?.copyWith(color: onSurfaceSubtle),
                    ),
                  ] else ...[
                    ListView.separated(
                      shrinkWrap: true,
                      ...
                      itemBuilder: (context, index) {
                        ...
                        final title = addr.municipio.isNotEmpty
                            ? addr.municipio
                            : (addr.departamento.isNotEmpty ? addr.departamento : addr.pais);
                        final subtitle = '${addr.direccion}${addr.municipio.isNotEmpty ? ', ${addr.municipio}' : ''}';
                        return AddressItem(
                          title: title,
                          subtitle: subtitle,
                          onTap: () async {
                  ...
                  SizedBox(
                    ...
                    child: FilledButton.icon(
                      onPressed: () async {
                        final uid = user.id;
                        final result = await context.push('${NamesRouter.createEditAddress}?userId=$uid');
                        if (result == true && context.mounted) {
