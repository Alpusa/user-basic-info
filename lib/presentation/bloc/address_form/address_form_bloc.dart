...
class AddressFormBloc extends Bloc<AddressFormEvent, AddressFormState> {
  ...
  AddressFormBloc(this._saveAddress, this._updateAddress, this._getAddressById, this._getUserById)
    ...
    on<AddressFormEvent>((event, emit) async {
      await event.map(
        ...
        departamentoChanged: (e) async => emit(state.copyWith(departamento: e.departamento)),
        municipioChanged: (e) async => emit(state.copyWith(municipio: e.municipio)),
        direccionChanged: (e) async => emit(state.copyWith(direccion: e.direccion)),
        submitPressed: (e) async => _onSubmitPressed(e.pais, e.departamento, e.municipio, e.direccion, emit),
      );
    });
  }

  Future<void> _onInitialized(String? addressId, String? userId, Emitter<AddressFormState> emit) async {
  ...
  Future<void> _onSubmitPressed(String? pais, String? departamento, String? municipio, String? direccion, Emitter<AddressFormState> emit) async {
    emit(state.copyWith(isSubmitting: true, submitted: true, failure: null, isSuccess: false));

    final valid = (pais != null && pais.isNotEmpty) &&
    ...
    if (state.isEdit && (state.id != null && state.id!.isNotEmpty)) {
      ...
      if (state.userId == null || state.userId!.isEmpty) {
        emit(state.copyWith(isSubmitting: false, failure: const Failure.validation('Falta el usuario para la dirección')));
        return;
      }
      final userRes = await _getUserById(state.userId!);
      final user = userRes.fold<du.User?>(
        (f) => null,
        (u) => u,
      );
      if (user == null) {
        emit(state.copyWith(isSubmitting: false, failure: const Failure.notFound('Usuario no encontrado')));
        return;
      }
